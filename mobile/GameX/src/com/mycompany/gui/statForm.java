...
public class statForm extends Form {
    ...
    // Set up the renderer
    int[] colors = new int[]{ColorUtil.BLUE, ColorUtil.MAGENTA, ColorUtil.GRAY};
 ...
 */
    ...
    List<datas> Form = new ArrayList<>();
     ArrayList<Category> tsk=ServiceCategory.getInstance().getAllCategories();
         
         for(Category form :tsk)
         {
     Form.add(new datas(0,  form.getId(),form.getName(),0));
         }
         for(datas stat :Form)
         {
             int t=0;
          for(Category abo :tsk)
          {
